...
const AdminNotifications = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Đã gửi thông báo đến tất cả khách hàng');
          ...
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Gửi tới tất cả khách hàng</p>
              </div>
            </div>
            ...
            {loadingInbox ? (
              ...
              <div className="space-y-4">
                {inbox.map((item) => (
                  <div 
                    key={item.id} 
                    className={`group relative flex gap-4 rounded-2xl border p-4 transition-all hover:bg-slate-50 ${item.isRead ? 'border-slate-100 bg-white' : 'border-blue-100 bg-blue-50/30 shadow-sm'}`}
                  >
