...
const ForgotPassword = () => {
  ...
  return (
    <div className="relative min-h-[80vh] flex items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(251,191,36,0.18),_transparent_30%),radial-gradient(circle_at_top_right,_rgba(14,165,233,0.14),_transparent_28%),linear-gradient(180deg,_#fff,_#f8fafc)] px-4 py-10">
          ...
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              <h2 className="text-2xl font-black text-slate-950">
                {hasResetLink ? 'Đặt lại mật khẩu' : 'Khôi phục tài khoản'}
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                {hasResetLink ? 'Tạo mật khẩu mới cho tài khoản của bạn.' : 'Nhập email để nhận link đặt lại mật khẩu.'}
              </p>
            </div>
          ...
          {!hasResetLink ? (
            <form onSubmit={handleSendResetLink} className="space-y-4">
              ...
              <div className="pt-2 text-center">
                <Link to="/login" className="text-sm font-semibold text-sky-700 hover:text-sky-800">
                  ← Quay lại đăng nhập
                </Link>
              </div>
            </form>
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-800">
                Link reset đã được xác thực cho <strong>{email}</strong>.
              </div>
              ...
              >
                {isResetting ? 'Đang đổi mật khẩu...' : 'Xác nhận đổi mật khẩu'}
              </button>
